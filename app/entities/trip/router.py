...
async def create_trip(schema: schemas.TripGot,
    ...
    return controller.Trip.create(schemas.TripCreate(**schema.dict(),
                                                     ...
                                                     available_seats=schema.max_passengers),
                                                     db=db).schema(db)


@trip_router.get("/as_driver")
async def get_trips(current_user: Annotated[User, Depends(User.get_current_user)],
                    db: Session = Depends(get_db)) -> list[schemas.TripReturn]:
    """Получаем наши поездки как водителя"""
    return controller.Trip.found_for_user(current_user.schema.id, db)
