...
async def create_car(schema: schemas.ReviewCreate,
    ...
    return _model_to_schema(ReviewCrud(db).create(schema, writer_id=current_user.id), db)
...
async def get_for_user(user_id: int, db: Session = Depends(get_db)):
    return _models_to_schema(ReviewCrud(db).get_by_user_id(user_id), db)
...
async def get_for_user(current_user: Annotated[UserReturn, Depends(get_current_user)],
    ...
    return _models_to_schema(ReviewCrud(db).get_by_user_id(current_user.id), db)
...
async def get_as_writer(current_user: Annotated[UserReturn, Depends(get_current_user)],
                       db: Session = Depends(get_db)):
    return _models_to_schema(ReviewCrud(db).get_by_writer_id(current_user.id), db)
